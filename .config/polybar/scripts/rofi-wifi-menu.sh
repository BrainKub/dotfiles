...
if [ -r "$DIR/wifi" ]; then
	source "$DIR/wifi"
elif [ -r "$HOME/.config/polybar/scripts/wifi" ]; then
	source "$HOME/.config/polybar/scripts/wifi"
else
...
CHENTRY=$(echo -e "$TOGGLE\nmanual\n$LIST" | uniq -u | rofi -dmenu -p "Wi-Fi SSID: " -lines "$LINENUM" -a "$HIGHLINE" -location "$POSITION" -yoffset "$YOFF" -xoffset "$XOFF" -font "$FONT" -width -4)
#echo "$CHENTRY"
